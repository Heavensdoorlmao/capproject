...
import csv
import random
from weapon import Shotgun, DestroyerCannon, Armyknife, Bow, Revolver, Katana
...
class World:
    ...
    def add_graphics(self):
        for row in self.world:
            for room in row:
                if isinstance(room, Room):
                    room.tile_map = TileMap(room, room.room_map, Spritesheet("spritesheet.png"))

    def assign_objects(self):
        for row in self.world:
            for room in row:
                if isinstance(room, Room):
                    if room.type == 'starting_room' and self.level == 1:
                        room.objects.append(Shotgun(self.game, room, (650, 300)))
                        room.objects.append(Revolver(self.game, room, (550, 300)))
                        room.objects.append(Katana(self.game, room, (750, 300)))
